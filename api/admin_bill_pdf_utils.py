...
class PDFGenerator:
    """Invoice PDF generator with fixed landscape layout and dynamic customer/items."""
    ...
    def _draw_invoice_meta(self, c, bill, x, y_top, w):
        ...
        c.drawString(x + 220, y + 7, f"Invoice Date: {bill.invoice_date.strftime('%Y-%m-%d')}")

        customer_name = self._safe(getattr(bill.customer, "name", ""))
        c.drawString(x + 420, y + 7, f"Customer: {customer_name}")
